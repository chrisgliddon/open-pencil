...
export interface GoogleFontInfo {
  family: string
  // Is this font actively downloaded / registered for this session?
  loaded: boolean
}

// Session-level registry of which Google Fonts have been successfully
// loaded (registered in document.fonts and CanvasKit).
...
export function getLoadedGoogleFontBytes(
  family: string,
  style = 'Regular'
): Promise<Uint8Array | null> {
  const variant = styleToVariant(style)
  const cacheKey = `gf:${family}|${variant}`
  return dbGet(cacheKey)
}
